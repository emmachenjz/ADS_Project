...
def print_result(result: dict) -> None:
    ...
    # Display up to 10 IDs inline; the complete list is always in result['subject_ids']
    print(f"→ Subject IDs (first 10 of {result['count']}): {_truncate_ids(result['subject_ids'])}")
    if "error" in result:
...
def main() -> None:
    ...
    mode = "LLM (claude-sonnet-4-6)" if api_key else "mock (keyword-based fallback)"
    print(f"=== ClinicalTrialDataAgent — Q4 Demo ===")
    print(f"Data   : {DATA_PATH}")
    print(f"Mode   : {mode}")
    if not api_key:
        print("Note   : No ANTHROPIC_API_KEY found — running in mock mode.")
        print("         Set the env var to use the live LLM. Mock mode exercises")
        print("         the full Prompt → Parse → Execute pipeline with keyword rules.")
    print("=" * 40)
