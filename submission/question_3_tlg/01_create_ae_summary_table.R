...
# suppressWarnings() suppresses the R-level "error in running command" warning so
# the log stays clean; residual shell messages (e.g. pandoc exit 127) are
# informational only and do not affect the required HTML output.
docx_ok <- tryCatch({
  ...
  message("Note: .docx export skipped — pandoc not available on this system.",
          " This is expected; HTML is the required deliverable.")
  FALSE
})
